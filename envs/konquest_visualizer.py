...
class KonquestVisualizer:
    ...
    def __init__(self,
        ...
        width = min(width // 1.5, int(self.BOARD_SIZE[0] * 0.70))
        ...
        self.__info = Info(self.__players, self.__planets, results, self.canvas)
...
class Info:
    ...
    def __init__(self,
                 players: List[Player],
                 planets: List[Planet],
                 results: Optional[Tuple[int, int]],
                 canvas: 'ResizableCanvas'):
        ...
        self.__add_results(results, players)
    ...
    def __add_results(self,
        ...
        x_offset = self.X_OFFSET + self.LENGTH / 2
        y_offset = 0.75 * self.__MARGIN + 10 * self.__PLANET_LENGTH

        self.__canvas.create_text(self.X_OFFSET + self.__MARGIN,
                                  y_offset,
                                  text="Result:",
                                  anchor=tk.NW,
                                  fill="white",
                                  size=35)
        y_offset += self.__MARGIN

        self.__canvas.create_text(x_offset - 2 * self.__MARGIN,
                                  y_offset,
                                  text=f"{results[0]}",
                                  anchor=tk.NW,
                                  fill=colors[players[0].id_],
...
class ResizableCanvas(tk.Canvas):
    ...
    def get_style(self, size: int):
        # return {"font": tkFont(size=int(size * self.y_scale))}
        return {"font": tkFont(family="Comic Sans MS", size=int(size * self.y_scale), weight='bold')}
...
if __name__ == "__main__":
    visualizer = KonquestVisualizer(Universe(["p1", "p2"], 6), 5, [3, 1])
