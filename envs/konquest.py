...
class Universe(AbstractState):
    ...
    def __init__(self,
                 ...
                 neutrals_count: int,
                 pre_existing_world: Optional[str]=None,
                 max_turns: int = __MAX_TURN):
        ...
        if pre_existing_world is None:
            self.__big_bang(neutrals_count)
        else:
            self.__replicate_world(pre_existing_world)
        self.__output_initial_state()
        ...
        for i in range(neutral_count):
            while True:
                position = (randrange(self.__SIZE[0]),
                            randrange(self.__SIZE[1]))
                for planet in self.planets:
                    # Two planets cannot be placed on the exact same location
    ...
    def __replicate_world(self, pre_existing_world: str):
        PLANET_TEMPLATE = r"# <Info: name = ([A-Z]), position = \(([0-9]), ([0-9])\), capacity = ([0-9]*), production = ([0-9\.]*)> *#"
        matcher = re.compile(PLANET_TEMPLATE)
        with open(pre_existing_world, encoding='utf-8') as input:
            for line in input:
                matched = matcher.match(line)
                if matched is None:
                    continue
                name, x, y, capacity, production = matched.groups()
                x = int(x)
                y = int(y)
                ...
                production = ''.join([p
                                        ...
                                        if (    i < production.find('.') + 3
                                            and p != '.')])
                production = int(production)
                planet_info = PlanetInfo(name, (x, y), capacity, production)
                self.planets.append(Planet(planet_info, ID.NEUTRAL, 0))
        return self

    def __output_initial_state(self):
        print("#{:#^72}#".format(""))
